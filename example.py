...
# data cleaninig
data['Sleep Disorder'] = data.apply(lambda row: 0 if row['Sleep Disorder'] == 'None' else 1, axis=1)
...
glem_class.preprocessing(numerical_preprocessing = 'std', categorical_preprocessing= 'label_encoder', splits = [0.7, 0.1, 0.2], random_state = 42)
...
glem_class.create_model([16,4], hidden_activation='tanh', output_activation='sigmoid', card_reduction=2)
...
train,_,_,_ = glem_class.train_model(epochs, folder_name, optimizer, scheduler, verbose, freeze, eps)
